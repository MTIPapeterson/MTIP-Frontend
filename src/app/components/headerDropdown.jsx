...
export default function HeaderDropdown({data}) {
    ...
    return (
        <div className='relative' onMouseOver={()=> setShowResources(true)} onMouseOut={() => setShowResources(false)}>
            ...
            {data?.data.resourcePages.nodes.map(page => <Link className="hover:underline hover:text-mt-blue-dark" key={page.resourcePageContent.pageName} href={`/resources/${page.resourcePageContent.pageName}`}>{page.title}</Link>) }
            </div>
            : ""}
        </div>
    )
